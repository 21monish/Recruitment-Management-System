					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
							...
							<tbody>
								<?php 
								$i = 1;
								$types = $conn->query("SELECT * FROM user_category where status = 1 order by id asc");
...
<script>
	...
	$('#manage-status-cat').submit(function(e){
		...
		start_load()
		$.ajax({
			url:'ajax.php?action=save_user_category',
	...
	$('.delete_scat').click(function(){
		_conf("Are you sure to delete this User Category","delete_scat",[$(this).attr('data-id')])
	})
	...
	function delete_scat($id){
		start_load()
		$.ajax({
			url:'ajax.php?action=delete_user_category',
			method:'POST',
			data:{id:$id},
			success:function(resp){
				if(resp==1){
