...
class Action
{
	...
	function save_user()
	{
		...
		if (empty($id)) {
			...
			$existing_check = $this->db->query("SELECT * FROM users WHERE name = '$name'")->num_rows;
			if ($existing_check > 0) {
				// If data already exists, return 0
				return 0;
			}
	...
	function save_vacancy()
	{
		...
		if (empty($id)) {
			// Check for existing data with the same email
			$existing_check = $this->db->query("SELECT * FROM vacancy WHERE position = '$position'")->num_rows;
			if ($existing_check > 0) {
				// If data already exists, return 0
				return 0;
			}
	...
	function save_application()
	{
		...
		if (empty($id)) {
			// Check for existing data with the same email
			$existing_check = $this->db->query("SELECT * FROM application WHERE email = '$email'")->num_rows;
			if ($existing_check > 0) {
				// If data already exists, return 0
				return 0;
			}
